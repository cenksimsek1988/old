...
public class Example implements GrwaConstants, CoreConstants{
	...
	// TODO
	private CoreHttpRequester<FredPriceRequestHelper> frHttpRequester; // bu da yukarıdaki gibi yapılacak..
	...
	public Example() {
		...
		// http requestleri yapacak bir obje lazım
		// bunu da sen kendi istediğin gibi oluşturursun, bu benim classımın nesnesi
		wbHttpRequester = new CoreHttpRequester<WorldBankPriceRequestHelper>() {
			@Override
			public void onAnswer(WorldBankPriceRequestHelper rHelper, boolean succcessfull, String answer) {
				if(succcessfull) {
					// handle edicez, bitince aşağıda yazılı recursive döngü ile 
					...
					// aynı risk faktör için ayrı http sorgusu atılması gereken ek sayfalar olabilir
					// bu yüzden handle ederken hem helper'ımı (yeni urli generate etsin)
					// hem de http request helper'ımı (dünya bankası için 
					handleWBResponse(rHelper, answer);
				} else {
					// benim http request helper classım http request fail olunca successfull parametresini
					// false döndürüyor
					...
					// parametre url yerine kendi clasımın objesini kullanıyorum, çok sayfalı api cevaplarını yönetmekte
					// yardımcı oluyor
					//TODO burada sen kendi http requestini tekrar edeceksin, belki timer koyarsın
					wbHttpRequester.request(rHelper);
				}
			}
		};
	...
	private void updateNextWB() {
		...
		String sql = rfsCenk.pickSql(SOURCE_WORLD_BANK, FREQUENCIES[frqIndex]);
		...
		if(riskFactors.isEmpty()) {
			if(frqIndex<FREQUENCIES.length-1) {
				frqIndex++;
				updateNextWB();
			} else {
				return;
	...
	private void updateFred() {
		// TODO
		// world bank gibi yapılacak
		FredPriceRequestHelper fRequester = new FredPriceRequestHelper(null, null, null);
		//TODO
		//TODO
		//................
		FredPriceHandler fHandler = new FredPriceHandler(fRequester.getSharedParams());
	}
}
